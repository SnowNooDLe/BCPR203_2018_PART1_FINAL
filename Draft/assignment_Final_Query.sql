...
SELECT
   ...
   COUNT(STR_TO_DATE(attendanceDate, "%M %d %Y")) AS 'Total attendance between 2018-02-08 and 2018-02-15'
FROM Student
...
SELECT
   ...
   MAX(STR_TO_DATE(attendanceDate, "%M %d %Y")) AS 'Last Attend Date',
...
SELECT
   CONCAT(firstName, ' ', lastName) AS 'Full Name',
   count(STR_TO_DATE(attendanceDate, "%M %d %Y")) AS 'Total attendance between their start date and today',
